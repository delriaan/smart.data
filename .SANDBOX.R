...
self$capture <- \(){
	...
	hist_name <- \(){ sprintf(
		"hist_%s"
		, rlang::env_names(self$history) |> length() |>
			(`+`)(1) |>
	...
	# Execution
	assign(hist_name(), self$smart.rules %$% mget(ls()) |> f(), envir = self$history)
}
